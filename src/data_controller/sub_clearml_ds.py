...
def get_local_dataset(dataset_input):
    ...
    minio_downloader = MinioDatasetDownloader(
        dataset=filtered_urls_by_category, download_dir="./datadir-debug"
    )
    ...
    print(
        "\nd_new_urls_by_cat:",
        {key: len(urls) for key, urls in filtered_urls_by_category.items()},
    )
