...
# Generate Graph
def generate_plot_one_vs_rest(class_names:list, 
                              gt_labels:list, 
...
def generate_plot_one_vs_one(class_names, gt_labels, 
    ...
    n_rows = int(np.ceil(n_combination / n_cols)) * 2 if int(np.ceil(n_combination / n_cols)) <= 10 else 10
    ...
    for idx in range(len(classes_combinations)):
        ...
        ax = plt.subplot(n_rows, n_cols, pos_top)
        sns.histplot(x = "prob", data = df_aux, 
                     hue = 'class', color = 'b', ax = ax, bins = bins)
